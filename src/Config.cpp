...
namespace SP {
  Config::Config(const std::string& filename)
    ...
    if (!conffile.is_open()) {
      throw std::runtime_error("Could not open FreiburgNetworkSyncBox config file.");
    }
    ...
    if ( ! std::getline(conffile, line) ) {
      throw std::runtime_error("First line missing in FreiburgNetworkSyncBox config file.");
    }
    subject = Trim(line);
    ...
    while (std::getline(conffile, line)) {
      ...
      split = SplitCSV(line);
      if (split.size() != 7) {
        throw std::runtime_error("Channel FreiburgNetworkSyncBox limits must all be 7 "
      ...
      if (limit.max_amp_uA == 0 || limit.max_freq_Hz == 0 ||
          limit.max_dur_ms == 0) {
        throw std::runtime_error("Zero maximum found in FreiburgNetworkSyncBox config.");
      }

      if ( ! (limit.area_mm_sq > 0 && limit.area_mm_sq < 25) ) {
        throw std::runtime_error("Electrode contact area out of range in "
            "FreiburgNetworkSyncBox config.");
      }

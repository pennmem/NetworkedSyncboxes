...
namespace SP {
  ...
  class Net {
    ...
    class Listener {
       ...
       */
      inline Listener(const std::string& port) {
        sockfd = Net_InvSock;
        ...
        if (listen(sfd, 128) == Net_SockErr) {
          throw std::runtime_error("Port in use");
        }

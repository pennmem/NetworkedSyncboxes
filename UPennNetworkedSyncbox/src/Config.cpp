...
namespace SP {
  Config::Config(const std::string& filename)
    ...
    if (!conffile.is_open()) {
      throw std::runtime_error("Could not open UPennNetworkSyncbox config file.");
    }
    ...
    if ( ! std::getline(conffile, line) ) {
      throw std::runtime_error("First line missing in UPennNetworkSyncbox config file.");
    }
    subject = Trim(line);
    ...
    while (std::getline(conffile, line)) {
      ...
      split = SplitCSV(line);
      if (split.size() != 7) {
        throw std::runtime_error("Channel UPennNetworkSyncbox limits must all be 7 "
